...
public static class LocationEndpoints {
    ...
    public static IEndpointRouteBuilder MapLocationEndpoints(this IEndpointRouteBuilder endpoints) {
        RouteGroupBuilder group = endpoints.MapGroup("/v1/locations");
        ...
        _ = group.MapPost("", async (
            ...
            IMongoRepository<UserLocation, string> repository) => {
                ...
                LocationPointRequest? invalidPoint = request.Locations.FirstOrDefault(p =>
        ...
        _ = group.MapGet("{userId}/last", async (
            string userId,
            IMongoRepository<UserLocation, string> repository) => {
                ...
                UserLocation? latest = await repository.Collection
                ...
                return latest is null
                    ? Results.NotFound()
                    : Results.Ok(new LastUserLocationResponse(
                    latest.UserId,
        ...
        _ = group.MapDelete("{userId}", async (
            string userId,
            IMongoRepository<UserLocation, string> repository) => {
                if (string.IsNullOrWhiteSpace(userId)) {
                    return Results.BadRequest(new { message = "userId is required." });
                ...
                DeleteResult result = await repository.Collection.DeleteManyAsync(x => x.UserId == userId);
    ...
    private static GeoJsonPoint<GeoJson2DGeographicCoordinates> toPoint(double latitude, double longitude) {
        return new(new GeoJson2DGeographicCoordinates(longitude, latitude));
    }
}
